...
class TopicRepository {
  ...
  addMessageToTopic(message: TopicMessage, topicId: string): Promise<TopicMessage> {
  ...
  async getLastTopics(): Promise<Topic[]>{
    const topicCreation = await this.prisma.topicCreation.findFirst({orderBy: {createdAt: 'desc'}});

    if(!topicCreation) {
        return [];
    }

    const topics = await this.prisma.topic.findMany({where: {topicCreationId: topicCreation.id}, include: {
        messages: true,
        topicCreation: true
    }});
